class Solution {
    ...
    public List<List<Integer>> kSmallestPairs(int[] nums1, int[] nums2, int k) {
        ...
        // Treat two arrays as a single matrix, where each cell represent the nums1[i] +
        // nums2[j]
        //   1 2 3
        // 4 5 6 7
        // 5 6 7 8
        ...
        // Post-processing
        long countL = countSmallerOrEqualTo(nums1, nums2, left);
        long countR = countSmallerOrEqualTo(nums1, nums2, right);

        // Must check countL first
        // If both countL and countR >= k, then left must be the answer
        if (countL >= k) {
            kthSum = left;
        } else if (countR >= k) {
            kthSum = right;
